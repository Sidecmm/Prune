...
simd<32> simdint_shr(const simd<32>& a, int b) {
    const uint32x4_t u =
        vshlq_u32(vreinterpretq_u32_s32(a), vdupq_n_s32(-b));
    return vreinterpretq_s32_u32(u);
}

simd<16> simd16_shr(const simd<16>& a, int b) {
    const uint16x8_t u =
        vshlq_u16(vreinterpretq_u16_s16(a),
...
simd<16> simd16_clamp(const simd<16>& value,
                      const simd<16>& min_val,
                      const simd<16>& max_val) {
    return vminq_s16(vmaxq_s16(value, min_val), max_val);
}

simd<16> simd16_uclamp(const simd<16>& value,
                       const simd<16>& min_val,
                       const simd<16>& max_val) {
    ...
    return vreinterpretq_s16_u16(
        vminq_u16(vmaxq_u16(v, mn), mx)
    );
}

simd<32> simdint_clamp(const simd<32>& value,
                       const simd<32>& min_val,
...
simd<16> simd16_maddubs(const simd<8>& a, const simd<8>& b) {
    ...
    const int16x8_t u_lo =
        vreinterpretq_s16_u16(
            vmovl_u8(vget_low_u8(u))
        );

    const int16x8_t u_hi =
        vreinterpretq_s16_u16(
            vmovl_u8(vget_high_u8(u))
        );

    const int16x8_t s_lo =
        vmovl_s8(vget_low_s8(b));

    const int16x8_t s_hi =
...
simd<8> simd8_broadcast32(uint32_t v) {
    return vreinterpretq_s8_s32(
        vdupq_n_s32(static_cast<int32_t>(v))
    );
}

simd<32> simdint_dpbusd(const simd<32>& acc,
                        const simd<8>& u,
                        const simd<8>& s) {
    const uint8x16_t uu =
        vreinterpretq_u8_s8(u);
    ...
    const int16x8_t p_hi =
        vmulq_s16(
            vreinterpretq_s16_u16(
                vmovl_u8(vget_high_u8(uu))
            ),
            vmovl_s8(vget_high_s8(s))
        );

    const int32x4_t summed =
        vpaddq_s32(
            vpaddlq_s16(p_lo),
            vpaddlq_s16(p_hi)
        );
...
simd<16> simd16_clamp(const simd<16>& value,
                      const simd<16>& min_val,
                      const simd<16>& max_val) {
    return ADDMM(min_epi16)(
        ADDMM(max_epi16)(value, min_val),
        max_val
    );
}
...
simd<16> simd16_uclamp(const simd<16>& value,
                       const simd<16>& min_val,
                       const simd<16>& max_val) {
    return ADDMM(min_epu16)(
        ADDMM(max_epu16)(value, min_val),
        max_val
    );
}
...
simd<32> simdint_clamp(const simd<32>& value,
                       const simd<32>& min_val,
                       const simd<32>& max_val) {
    return ADDMM(min_epi32)(
        ADDMM(max_epi32)(value, min_val),
        max_val
    );
}
...
int mysum(const simd<32>& x) {
    ...
    const auto sum128 =
        _mm_add_epi32(high128, low128);

    const auto high64 =
        _mm_unpackhi_epi64(sum128, sum128);

    const auto sum64 =
        _mm_add_epi32(sum128, high64);
    ...
    const auto high64 =
        _mm_unpackhi_epi64(x, x);

    const auto sum64 =
        _mm_add_epi32(x, high64);
